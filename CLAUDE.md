...
- **Database is driver-agnostic, chosen entirely by `DATABASE_URL`** (`db/database.py`): SQLite for local dev, PostgreSQL (psycopg3) in production. `_normalize_db_url` rewrites Render's `postgres://` / `postgresql://` schemes to `postgresql+psycopg://` (SQLAlchemy 2.0 would otherwise pick psycopg2, which isn't installed). SQLite gets `check_same_thread=False`; Postgres gets `pool_pre_ping` + `pool_recycle`. Because of this, anything DB-specific must stay dialect-safe — e.g. boolean `server_default` uses `expression.false()/true()` (renders `0/1` on SQLite, `false/true` on Postgres), and `migrate_admin_flag` switches its ALTER literal on `engine.dialect.name`.
...
- **OAuth** (`routers/auth.py`, `services/oauth_service.py`): Google / Kakao / Naver / Apple are all implemented end-to-end. A provider's `/auth/<p>/login` returns 501 only when its credentials are absent from `.env` (the `if not settings.<p>_client_id` guard). Flow ends by redirecting to the frontend with the JWT in the URL hash. PII the provider exposes under user consent (name/phone/birthday — Kakao & Naver) is back-filled into the encrypted User columns on every login via `get_or_create_oauth_user`.
